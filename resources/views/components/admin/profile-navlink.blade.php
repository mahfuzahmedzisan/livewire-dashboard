...
@if ($logout)
    ...
    <form id="logout-form" action="{{ $route }}" method="POST" class="mx-1 block">
        ...
        <button type="submit" class="block w-full px-4 py-2 text-text-dark-secondary dark:text-text-white hover:bg-bg-black/10 dark:hover:bg-bg-white/10 transition-colors rounded-md text-left">
            {{ $name }}
        </button>
